...
class BudgEngagement(models.Model):
	...
	def action_eng_confirmer(self):
		...
		if self.mnt_eng > res:
			...
			and cd_article_id = %d and cd_paragraphe_id = %d and cd_rubrique_id = %d and x_exercice_id = %d and company_id = %d and state not in ('A'))
	...
	def cumul_anterieur(self):
		...
		and x_exercice_id = %d and state not in ('A') """ %(val_para, val_rub, val_struct, val_ex))
		...
		and x_exercice_id = %s and no_eng = %s and state not in ('A') """ ,(val_para, val_rub, val_struct, val_ex, v_eng))
	...
	def mnt_sum_eng(self):
		...
		self.env.cr.execute("""select sum(e.mnt_eng) FROM budg_engagement e
				...
				and e.company_id = %d and e.x_exercice_id = %d and state not in ('A')""" %(val_para, val_rub, val_struct, val_ex))
		res = self.env.cr.fetchone()
...
class BudgLiqOrd(models.Model):
	...
	no_eng = fields.Many2one("budg_engagement", "Engagement", required=True,domain=[('state', '=', 'W')],  states={'N': [('readonly', True)], 'L': [('readonly', True)], 'A' :[('readonly', True)]})
...
class Ordre_Paiement(models.Model):
	...
	imput_benefs = fields.Integer(string="Valeur", required=False)
...
class BudgBordMdtRecepDcmef(models.Model):
	...
	def afficher(self):
		...
		for vals in self:
			vals.env.cr.execute("""select m.dt_etat as dte, m.id as mdt, m.cd_titre_id as titre, m.no_lo as liq, m.cd_section_id as sec, m.cd_chapitre_id as chap,
	...
	def envoyer(self):
		...
		for vals in self:
			...
			self.env.cr.execute("""select count(l.*) from budg_liste_mandatement_recep l, budg_bordereau_mandatement_recep_dcmef b where l.envoyer = True and l.liste_id = %d
						and b.id = l.liste_id""" % (id_b))
			res = self.env.cr.fetchone()
			resu = res and res[0] or 0
			if resu < 1:
				raise ValidationError(_("Cochez au moins une ordonnance"))
			else:
...
class BudgCorrectionEngagementApprobation(models.Model):
	...
	eng_id = fields.Many2one("budg_engagement", "N° Engagement",required=True, domain=['|',('state','=','V'),('state','=','L')])
	titre_id = fields.Many2one("budg_titre", "Titre", readonly=True)
	section_id = fields.Many2one("budg_section", "Section", readonly=True)
	chapitre_id = fields.Many2one("budg_chapitre", "Chapitre", readonly=True)
	article_id = fields.Many2one("budg_param_article", "Article", readonly=True)
...
class BudgListeOp(models.TransientModel):
	_name = "budg_liste_op"

	name = fields.Char(default="Liste des OP")
	date_deb = fields.Date("Du")
	...
	company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id.id)
	op_ids = fields.One2many("budg_liste_line", "liste_id", readonly=True)

	def afficher(self):
		v_ex = int(self.x_exercice_id)
		v_struct = int(self.company_id)
		dt_deb = self.date_deb
		dt_fin = self.date_fin

		if self.date_deb and self.date_fin:
			for vals in self:
				vals.env.cr.execute("""select * from budg_op b
				where b.company_id = %s and b.x_exercice_id = %s and b.date_emis between %s and %s""",
									(v_struct, v_ex, dt_deb, dt_fin))
				...
				vals.op_ids.unlink()
				for line in rows:
					result.append((0, 0, {'dt_op': line['date_emis'], 'no_op': line['no_op'],
										  'beneficiaire': line['no_beneficiaire'], 'objet': line['lb_obj'],
										  'montant': line['mnt_op'], 'type_paiement': line['type_operation']}))
				self.op_ids = result
		else:
			for vals in self:
				vals.env.cr.execute("""select * from budg_op b
				where b.company_id = %s and b.x_exercice_id = %s""", (v_struct, v_ex))
...
class BudgListeLine(models.TransientModel):
	_name = "budg_liste_line"

	liste_id = fields.Many2one("budg_liste_op", ondelete='cascade')
	dt_op = fields.Date("Date op")
	no_op = fields.Many2one("budg_op","N° OP")
	objet = fields.Text("Objet")
	beneficiaire = fields.Many2one("ref_beneficiaire", "Bénéficiaire")
	montant = fields.Float("Montant")
	type_paiement = fields.Many2one("budg_typeordrepaiement", "Type ordre paiement")
