...
class Compta_Ecriture(models.Model):
    ...
    def gen_ecr_libr(self):
        for val in self:
            ...
            vals = self.no_ecr
            self.env.cr.execute("UPDATE compta_ligne_ecriture SET no_ecr = %s WHERE ecriture_id = %s" ,(vals, val_id))
...
class Compta_Paiement(models.Model):
    ...
    date_op = fields.Date("Emis le", default=fields.Date.context_today)
    ...
    def generer_ecriture(self):
        ...
        if resu > 0:
            ...
            self.env.cr.execute("select * from compta_paiement_dep where company_id = %d and id = %d" %(val_struct, id_paie))
            ...
            self.env.cr.execute(
                "select no_lecr from compta_compteur_ecr where x_exercice_id = %d and company_id = %d" % (val_ex, val_struct))
            noecr = self.env.cr.dictfetchall()
            no_ecrs1 = noecr and noecr[0]['no_lecr']
            ...
            self.env.cr.execute("select * from compta_cheq_dep where company_id = %d and paiement_id = %d " %(val_struct, id_paie))
            for val in self.env.cr.dictfetchall():
                no_ecr = val['no_ecr']
                no_lecr = val['no_lecr']
                ...
                if var_cptes == 0:
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'P') """, (no_ecr, no_lecr, var_cptes, type_pj, ref_pj, mnt, v_ex, val_struct, dte, fg_sens))
        ...
        for vals in self.detail_ov_ids:
            ...
            if vals.type_pj.libelle.refe == '31':
                self.env.cr.execute("update budg_mandat set state = 'F' where x_exercice_id = %s and company_id = %s and id = %s" ,(val_ex, val_struct, val_mdt))
            elif vals.type_pj.libelle.refe == '30':
                self.env.cr.execute("update budg_op set et_doss = 'F' where x_exercice_id = %s and company_id = %s and id = %s" ,(val_ex, val_struct, val_op))
            ...
            if resultat != 0:
                ...
                vals.env.cr.execute("""SELECT * from compta_retenue_cheq_emis l where l.numcheq = %d and l.company_id = %d and l.x_exercice_id = %d and l.state  = 'V' """ %(id_paie, val_struct, val_ex ))
...
class Compta_op_releve(models.Model):
    ...
    def action_confirmer(self):
        ...
        self.env.cr.execute("""select coalesce (sum(case when l.code1 like '%%%s%%' then l.montant end),0) as mntdebit,
        ...
        from compta_op_releve r, compta_op_releve_lines l
        where r.id = l.op_releve_id and l.op_releve_id = %d and l.company_id = %d""" %(v_d, v_e, v_d, v_e, v_id, v_struct))

        #self.env.cr.execute("""select coalesce(sum(l.montant),0) from compta_op_releve r, compta_op_releve_lines l
        #where l.code1 like '%%%s%%' and r.id = l.op_releve_id and l.op_releve_id = %d and l.company_id = %d""" %(v_e,v_id, v_struct))

        #mtr = self.env.cr.fetchone()
        #self.montant_recette = mtr and mtr[0] or 0
    ...
    def generer_ecriture(self):
        ...
        self.env.cr.execute("""SELECT code1,id_imput, montant, type_pj FROM compta_op_releve_lines WHERE company_id = %d AND op_releve_id = %d """ %(val_struct, v_id))
        ...
        for line in self.env.cr.dictfetchall():
            ...
            self.env.cr.execute(
                "UPDATE compta_op_releve_lines SET no_ecr = %d WHERE op_releve_id = %d and x_exercice_id = %d AND company_id = %d " % (no_ecr,v_id, val_ex, val_struct))
...
class Compta_balance_reel(models.Model):
    ...
    def afficher(self):
        ...
        for vals in self:
            vals.env.cr.execute("""select r.souscpte as num,r.id as ids,r.concate_souscpte as compte from ref_souscompte r, compta_plan_lines l 
            where r.id = l.souscpte order by num asc""")
            ...
            vals.balance_lines.unlink()
            for line in rows:
                result.append((0,0, {'entier_cpte' : line['num'], 'entier' : line['ids'], 'numero_compte' : line['compte']}))
            self.balance_lines = result
    ...
    def remplir_balance_reel(self):
        ...
        self.env.cr.execute("""select l.no_souscptes as ids,sum(l.mt_lecr) as debiteur from compta_ligne_ecriture l 
        where l.dt_ligne between %s and %s and l.company_id = %s and l.x_exercice_id = %s and l.fg_etat not in ('A','R')
        and l.no_souscptes <> 0 and l.fg_sens = 'D' group by l.no_souscptes""" ,(val_deb, val_fin, val_struct, val_ex))
        ...
        for val in self.env.cr.dictfetchall():
            ...
            self.env.cr.execute("UPDATE compta_balance_reel_detail SET debiteur = %s where entier = %s and balance_reel_id = %s and company_id = %s" %(mt_d, cpte, v_id, val_struct))

        self.env.cr.execute("""select l.no_souscptes as ids,sum(l.mt_lecr) as crediteur from compta_ligne_ecriture l 
        where l.dt_ligne between %s and %s and l.company_id = %s and l.x_exercice_id = %s and l.fg_etat not in ('A','R')
        and l.no_souscptes <> 0 and l.fg_sens = 'C' group by l.no_souscptes""",
                            (val_deb, val_fin, val_struct, val_ex))

        for va in self.env.cr.dictfetchall():
            cpte = va['ids']
            mt_c = va['crediteur']

            self.env.cr.execute(
                "UPDATE compta_balance_reel_detail SET crediteur = %s where entier = %s and balance_reel_id = %s and company_id = %s" % (
                mt_c, cpte, v_id, val_struct))
...
class Compta_balance_entree(models.Model):
    ...
    def gen_balance(self):
        ...
        #self.env.cr.execute("INSERT INTO compta_ecriture(no_ecr, dt_ecriture, type_ecriture, x_exercice_id, company_id, state) VALUES (%s, %s, %s, 'L', %s, %s, 'P')" ,(self.no_ecr, val_date, val_ecr, val_ex, val_struct))

        self.env.cr.execute("INSERT INTO compta_ecriture(no_ecr,dt_ecriture, type_ecriture, type_op ,x_exercice_id, company_id, state) VALUES (%s, %s, %s,'L', %s, %s, 'P')",(self.no_ecr, val_date, val_ecr, val_ex, val_struct))
        ...
        self.env.cr.execute("""update compta_balance_entree_line set no_ecr = %s where entree_id = %s""",(self.no_ecr,v_id))
    ...
    def remplir_tenue(self):
        ...
        if self.compte:
            ...
            for vals in self:
                ...
                and l.company_id = %s and l.fg_etat not in ('A','R') order by l.no_ecr, l.no_lecr """ ,(val_cpte, val_ex, val_struct))
                rows = vals.env.cr.dictfetchall()
                result = []
            ...
            for vals in self:
                ...
                and l.x_exercice_id = %s AND l.company_id = %s and l.fg_etat not in ('A','R') group by etat""" ,(val_cpte, val_ex, val_struct))
                rows = vals.env.cr.dictfetchall()
                result = []
        ...
        if self.compte and self.fg_sens:
            for vals in self:
                ...
                from compta_ligne_ecriture l, compta_plan_lines c, ref_souscompte r where c.souscpte = l.no_souscptes and r.id = c.souscpte and l.no_souscptes = %s and l.x_exercice_id = %s
                and l.company_id = %s and l.fg_sens = %s order by l.no_ecr, l.no_lecr""" ,(val_cpte, val_ex, val_struct, val_sens))
                rows = vals.env.cr.dictfetchall()
                result = []
        ...
        if self.tout_compte:
            for vals in self:
                vals.env.cr.execute("""select distinct l.no_lecr as noligne, l.no_ecr as noecr, l.dt_ligne as dte_ligne ,l.fg_sens as sens, l.mt_lecr as mnt,l.dt_valid as valid,l.dt_verif as verif, l.fg_etat as etat 
                from compta_ligne_ecriture l, compta_plan_lines c, ref_souscompte r where c.souscpte = l.no_souscptes and r.id = c.souscpte and l.x_exercice_id = %s and l.company_id = %s and l.fg_etat not in ('A','R') order by l.no_ecr, l.no_lecr order by l.no_ecr, l.no_lecr asc """ ,(val_ex, val_struct))
                rows = vals.env.cr.dictfetchall()
                result = []
            ...
            for vals in self:
                ...
                and l.x_exercice_id = %s AND l.company_id = %s and l.fg_etat not in ('A','R') group by etat""" ,(val_ex, val_struct))
                rows = vals.env.cr.dictfetchall()
                result = []
        ...
        if self.tout_compte and self.etat:
            for vals in self:
                ...
                and l.company_id = %s and l.fg_etat = %s order by l.no_ecr, l.no_lecr""" ,(val_ex, val_struct, val_etat))
                rows = vals.env.cr.dictfetchall()
                result = []
...
class Compta_edition_ecriture(models.TransientModel):
    ...
    def chercher_ecriture(self):
        ...
        for vals in self:
            ...
            and l.company_id = %s and l.x_exercice_id = %s and no_ecr = %s
            group by r.souscpte, r.lb_long, ligne, sens, mnt, etat, type_pj order by l.no_lecr""",
                                (val_struct, val_ex, val_ecr))
            rows = vals.env.cr.dictfetchall()
            result = []
...
class Compta_balance_generale(models.TransientModel):
    ...
    def afficher_balance(self):
        ...
        for vals in self:
            vals.env.cr.execute(""" select l.souscpte as scpte, initcap(r.lb_long) as lib, r.souscpte as cpte from compta_plan_lines l, ref_souscompte r where r.id = l.souscpte order by r.souscpte """)
    ...
    def Calcul(self):
        ...
        for vals in self.env.cr.dictfetchall():
            ...
            and l.company_id = %d and l.x_exercice_id = %d and l.fg_etat not in ('A','R') and l.mt_lecr <> 0 group by r.souscpte, r.lb_long order by compte asc""" %(cpte,val_struct, val_ex))
...
class Compta_balance_generale_line(models.TransientModel):
